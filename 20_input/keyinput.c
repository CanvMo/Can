...
    dev->curkeynum = 0;
    dev->timer.data = (volatile long)dev_id;   //传入 timer_function 的参数
...
        printk("input_report_key press\r\n");
    } else {                                     /* 按键松开 */
        //input_event(dev->inputdev, EV_KEY, keydesc->value, 0);
        input_report_key(dev->inputdev, keydesc->value, 0);
        input_sync(dev->inputdev);
        printk("input_report_key release\r\n");
    }    
}
...
    else
    {
        printk("register input device success\r\n");
    }
